...
class takeAssessment extends Component {
          ...
          ({ id, name, description, answer_options, question_type }, index) => (
            <div key={id} className="assessment_question">
              <h3>
                ...
                <span>{res.data.questions.length}</span>
              </h3>
              <br />
  ...
  fetchUserAnswers = () => {
    ...
    axios
      ...
      .then(res => {
        res.data.map(answer => {
          selections[answer.question.id] =
            answer.question.question_type === "choice"
  ...
  getKeyByValue(object, value) {
    return Object.keys(object).find(
      key => this.searchArray(value, object[key])["id"] === value
    );
  }
    ...
    if (question_type === "theory") {
      elements = (
        <div className="form-group">
          <label className="col-lg-8 adjust-input control-label">Content</label>
        ...
        >
          {answer_options.map(item => (
            <div key={item.id} className="col-md-6">
              <div
                className={
                  this.realSelectedValue(id) === item.id
                    ? "card selected"
                    : "card"
                }
                id={`card_${item.id}`}
